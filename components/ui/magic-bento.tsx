...
export interface BentoProps {
  ...
  cards?: readonly BentoCardTranslation[];
}
...
const cardData: BentoCardProps[] = [
  ...
  },
  {
    ...
    description: 'Gain the advantage over your competitors',
    label: 'Opportunities',
...
const CARD_ICONS = [CitationTrackingIcon, SentimentAnalysisIcon, QueryPatternsIcon, TrendingQueriesIcon];
...
const MagicBento: React.FC<BentoProps> = ({
  ...
  const displayCards: BentoCardProps[] = cardsOverride
    ? cardsOverride.map((c, i) => ({
        ...cardData[i],
        label: c.label,
        title: c.title,
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {displayCards.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative aspect-[4/3] min-h-[250px] w-full max-w-full p-6 rounded-2xl border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out ${
            ...
            if (enableStars) {
              return (
                ...
                >
                  <div className="card__header flex justify-between gap-3 relative z-10" style={{ color: 'white' }}>
                    <span className="card__label text-base font-medium">{card.label}</span>
                  </div>
                  <div className="card__content flex flex-col relative z-10" style={{ color: 'white' }}>
